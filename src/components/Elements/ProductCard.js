...
export const ProductCard = ({product}) => {
  ...
  <Link to={`/products/${id}`} className="relative">
    {best_seller && <span className="absolute top-2 left-2 bg-yellow-500 text-white text-xs font-semibold px-2 py-1 rounded">Best Seller</span>}
    <img
  ...
  <div className="p-5">
    ...
    <p className="flex justify-between items-center">
      ...
      <button className="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800">
        Enroll Now <i className="ml-1 bi bi-plus-lg"></i>
      </button>
